...
/// <summary>
/// Implements a WPF/MAUI-style multi-binding framework.
/// </summary>
/// <remarks>
/// NOTE: Currently only one-way bindings (from source to target) are supported.
/// </remarks>
...
public class MultiBinding
{
        ...
        new PropertyMetadata(null, (sender, e) =>
        {
            ...
            foreach (var mb in mbc)
            {
                if (string.IsNullOrEmpty(mb.PropertyName))
                    throw new InvalidOperationException($"{nameof(mb.PropertyName)} required when instantiating a {nameof(MultiBinding)} from XAML.");

                var dp = mb._resolvedProperty ?? (mb._resolvedProperty = 
                    (mb.PropertyOwner ?? fe.GetType()).TryGetDependencyProperty(mb.PropertyName));

                if (dp == null)
                    throw new InvalidOperationException($"Could not locate {mb.PropertyName}Property on {fe.GetType()}; did you forget to specify {nameof(mb.PropertyOwner)}?");
    ...
    private class MultiBindingExpression : INotifyPropertyChanged
    {
        ...
        public object FinalValue
        {
            ...
            set
            {
                if (_FinalValue == value)
                    return;
                _FinalValue = value;
                this.PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(FinalValue)));
            }
        }
        ...
        internal void Reevaluate()
        {
            if (_isApplying || this.Target?.IsLoaded != true)
                return;
            this.FinalValue = this.MultiBinding.Converter.Convert(this.ProxyValues, this.MultiBinding.ConverterParameter);
        }
        ...
        static readonly GrowableArray<DependencyProperty> _proxyProps = new GrowableArray<DependencyProperty>(10); // not thread safe
        static ConditionalWeakTable<FrameworkElement, ProxyCount> _proxiesUsed = new ConditionalWeakTable<FrameworkElement, ProxyCount>();   // already thread safe
        bool _isApplying;
    }
    ...
    private class MultiBindingProxyConverter : IValueConverter
    {
        ...
        public MultiBindingProxyConverter(MultiBindingExpression expression, MultiBindingProxy proxy)
        {
            _proxy = proxy;
        ...
        {
            // TODO: TwoWay support
            System.Diagnostics.Debug.WriteLine("[WARNING] TwoWay support is not implemented.");
            return null;
        }
    }
}
...
/// <summary>
/// Support for <see cref="MultiBinding"/> routines.
/// </summary>
public static class BindingExtensions
{
    public static DependencyProperty? TryGetDependencyProperty(this Type type, string propertyName)
    {
        // In WinUI the DP instances of native objects are exposed as properties rather than fields.
        var dpProp = type.GetProperty($"{propertyName}Property", BindingFlags.Static | BindingFlags.FlattenHierarchy | BindingFlags.Public);
        if (dpProp != null)
            return dpProp.GetValue(null) as DependencyProperty;

        var dpField = type.GetField($"{propertyName}Property", BindingFlags.Static | BindingFlags.FlattenHierarchy | BindingFlags.Public);
        if (dpField != null)
